...
/* The core offers to be rendered on the frontend's audio thread, which is
 * the desktop's arrangement: the sink's clock is the only one. */
UTEST(run, the_core_offers_to_be_pulled)
{
    load(utest_result, ROM("mode3_8bpp"));
    ASSERT_TRUE(fe.audio_cb.callback != NULL);
    ASSERT_TRUE(fe.audio_cb.set_state != NULL);
    fe.unload_game();
}

/* While the frontend's audio driver is active the frame hands over no sound
 * at all; each pull is the count the core chose. When the driver goes idle
 * the frame carries it again. */
UTEST(run, a_pulled_core_leaves_the_frame_alone)
{
    load(utest_result, ROM("mode3_8bpp"));
    fe.audio_cb.set_state(true);
    fe.audio_calls = 0;
    fe.audio_frames = 0;
    fe_run(60);
    ASSERT_EQ(fe.audio_calls, 0);
    fe.audio_cb.callback();
    ASSERT_EQ(fe.audio_calls, 1);
    ASSERT_EQ(fe.audio_frames, (size_t)256);
    fe.audio_cb.set_state(false);
    fe_run(1);
    ASSERT_EQ(fe.audio_calls, 2);
    ASSERT_EQ(fe.audio_frames, (size_t)(256 + 800));
    fe.unload_game();
}

/* What the audio thread pulls is the program's music, converted the same way
 * the frame's push is. */
UTEST(run, what_is_pulled_is_heard)
{
    ASSERT_TRUE(fe_load(FIXTURES_DIR "/furelise.rp6502"));
    fe.audio_cb.set_state(true);
    fe.audio_peak = 0;
