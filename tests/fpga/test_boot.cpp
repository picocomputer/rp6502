 ...
 * reset, and the 6502 prints. No testbench backdoor into the 6502's world —
 * only the firmware image goes in, the way a bitstream carries it.
 */
...
UTEST(boot, firmware_boots_the_6502)
{
    ...
    bool stopped = false;
    for (int i = 0; i < 400000 && !stopped; i++)
    {
        dut->clk_sys = 1;
        dut->eval();
        dut->clk_sys = 0;
