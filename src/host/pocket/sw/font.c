 ...
 * lanes, so a word is built from four byte reads. */
...
/* The glyphs at VID_FONT16 and VID_FONT8 are stored row-major, with one
 * byte for each character code in a row, so row r of codes 128 to 255
 * starts at byte r * 256 + 128. In the asset, a code page's rows are 128
 * bytes apart. */
...
/* The savestate does not include the font store, so the store is rebuilt
 * from the asset and font_code_page, which is in the TCM that the
