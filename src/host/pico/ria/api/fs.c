...
// File descriptor pool for open files, and one more beyond it that only the
// ROM loader is given -- a program can neither name it nor be handed it,
// because fs_std_open never counts that high.
#define FAT_FIL_MAX 8
#define FAT_FIL_ROM FAT_FIL_MAX
static FIL fat_fil_pool[FAT_FIL_MAX + 1];

static FIL *fat_validate_fil(int desc)
{
    if (desc < 0 || desc > FAT_FIL_ROM)
        return NULL;
...
{
    FIL *fp = &fat_fil_pool[FAT_FIL_ROM];
    if (fp->obj.fs) /* one at a time: a new program replaces the last */
    {
        f_close(fp);
        fp->obj.fs = NULL;
    }
    FRESULT fresult = f_open(fp, path, FA_READ);
    if (fresult != FR_OK)
    {
