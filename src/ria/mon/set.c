...
static void set_print_keyb()
{
    const char *const keyb_labels[] = {"0 - US-Layout", "1 - German-Layout"};
    printf("KEYB : %s\n", keyb_labels[cfg_get_keyb()]);
}

static void set_keyb(const char *args, size_t len)
{
   uint32_t val;
    if (len)
    {
