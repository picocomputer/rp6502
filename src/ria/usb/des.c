...
static const pad_descriptor_t *detect_sony_ds4_controller(uint16_t vendor_id, uint16_t product_id)
{
...
static const pad_descriptor_t *detect_sony_ds5_controller(uint16_t vendor_id, uint16_t product_id)
{
                                 ...
                                 uint16_t vendor_id, uint16_t product_id)
{
    ...
    found = detect_sony_ds4_controller(vendor_id, product_id);
    if (found)
    {
    ...
    found = detect_sony_ds5_controller(vendor_id, product_id);
    if (found)
    {
    ...
    if (!desc->valid)
    {
        DBG("Detected generic controller, parsing HID descriptor.\n");

        // Initialize button_offsets array with 0xFFFF (unused markers)
