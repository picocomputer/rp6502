...
// rln_set_term_* override. (0,0) clears it. A mid-edit change reflows the
// current input line at the new width.
void rln_set_naws_size(uint16_t w, uint16_t h);

// Terminal width. Priority: rln_set_term_width override if set, then the
// telnet NAWS width, then the most recent CPR-reported width, then a
// VGA-aware fallback (40 for 320-wide canvases, 80 otherwise) when VGA is
// connected, then 80.
uint16_t rln_get_term_width(void);

// Terminal height. Priority: rln_set_term_height override if set, then the
// telnet NAWS height, then the most recent CPR-reported height, then a
// VGA-aware fallback (32 in display mode 2, 30 otherwise) when VGA is
// connected, then 24.
