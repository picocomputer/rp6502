 ...
 * table (open/close/read/write/lseek/sync over a small FIL pool). FatFs-only —
 * the block device (diskio) is supplied by the platform (usb/msc.c on hardware,
 * a RAM disk in the emulator), so this file is shared with the emulator.
 */
