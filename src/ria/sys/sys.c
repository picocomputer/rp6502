...
#define X(suffix, name, tz)                            \
    static const char __in_flash("sys_tzinfo_strings") \
        SYS_TZINFO_NAME_##suffix[] = name;             \
    static const char __in_flash("sys_tzinfo_strings") \
        SYS_TZINFO_TZ_##suffix[] = tz;
SYS_TZINFO
#undef X

#define X(suffix, name, tz) \
    SYS_TZINFO_NAME_##suffix,
static const char *__in_flash("sys_tzinfo_name")
    sys_tzinfo_name[] = {SYS_TZINFO};
#undef X

#define X(suffix, name, tz) \
    SYS_TZINFO_TZ_##suffix,
static const char *__in_flash("sys_tzinfo_tz")
    sys_tzinfo_tz[] = {SYS_TZINFO};
#undef X

#define SYS_TZINFO_COUNT (sizeof(sys_tzinfo_name) / sizeof(*sys_tzinfo_name))

static uint64_t sys_start_us;
static int sys_tzinfo_index;
...
void __in_flash("sys_init") sys_init(void)
{
    ...
    aon_timer_start(&ts);
    // cfg_init runs later and overrides this through sys_load_time_zone.
    setenv(STR_TZ, sys_tzinfo_tz[sys_tzinfo_index], 1);
    tzset();
#ifdef NDEBUG
...
static bool sys_strftime_worker(char *dst, size_t *pos, size_t max,
                                ...
                                int depth)
{
    while (*format)
    {
        ...
        {
            if (depth >= 2 ||
                !sys_strftime_worker(dst, pos, max, loc_format, tm, depth + 1))
                return false;
        }
        else
        {
                    ...
                    const struct tm *tm)
{
    ...
    if (!max)
        return 0;
    if (!sys_strftime_worker(dst, &pos, max, format, tm, 0))
        pos = 0;
    dst[pos] = 0;
    return pos;
}

int sys_status_response(char *buf, size_t buf_size, int state, unsigned)
{
    (void)state;
    struct timespec ts;
    ...
    sys_strftime(buf, buf_size, STR_STATUS_TIME, &tminfo);
    return -1;
}

int sys_tzdata_response(char *buf, size_t buf_size, int state, unsigned)
{
    if (state < 0)
        return state;
    size_t cell = 0;
    for (unsigned i = 0; i < SYS_TZINFO_COUNT; i++)
    {
        size_t len = strlen(sys_tzinfo_name[i]);
        if (len > cell)
            cell = len;
    }
    ...
    if (cols < 1)
        cols = 1;
    unsigned rows = (SYS_TZINFO_COUNT + cols - 1) / cols;
    ...
    unsigned el = state;
    for (unsigned i = 0; i < cols && el < SYS_TZINFO_COUNT; i++)
    {
        n = snprintf(buf, buf_size, "%-*s%*s",
                     (int)cell, sys_tzinfo_name[el], (int)spread, "");
        if (n > 0 && (size_t)n < buf_size)
        {
            buf += n;
            buf_size -= n;
...
void sys_load_time_zone(const char *str)
{
    ...
    for (unsigned i = 0; i < SYS_TZINFO_COUNT; i++)
    {
        if (!strcasecmp(tz, sys_tzinfo_name[i]))
        {
            sys_tzinfo_index = i;
            setenv(STR_TZ, sys_tzinfo_tz[i], 1);
            tzset();
            return;
        }
    }
...
{
    if (strlen(tz) >= SYS_TZ_MAX_SIZE)
        return false;
    int found_index = -1;
    for (unsigned i = 0; i < SYS_TZINFO_COUNT; i++)
    {
        const char *tzname = sys_tzinfo_name[i];
    ...
    if (found_index != sys_tzinfo_index ||
        (found_index < 0 && sys_tzinfo_index < 0 &&
         (!current_tz || strcmp(current_tz, tz))))
    {
        sys_tzinfo_index = found_index;
        if (sys_tzinfo_index < 0)
            setenv(STR_TZ, tz, 1);
        else
            setenv(STR_TZ, sys_tzinfo_tz[sys_tzinfo_index], 1);
        tzset();
...
const char *sys_get_time_zone(void)
{
    if (sys_tzinfo_index < 0)
        ...
        return sys_tzinfo_name[sys_tzinfo_index];
}
...
void sys_mon_status(const char *args)
{
    ...
    mon_add_response_fn(sys_status_response);
