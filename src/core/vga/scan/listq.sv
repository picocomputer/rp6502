 ...
 * slot free and the queue and the two clocks in flight can take one, up
 * to the list's last word. A descriptor is read out of the queue's first
...
module listq (
    ...
    /* How many words the queue may hold for this engine, and whether the
     * words a pop frees on this clock count as room. */
    input logic [3:0] cap,
    ...
    always_comb begin
        ...
        listq_req = active && dfp <= dend
            && dqn + {3'd0, land} + {3'd0, flight}
               - (pop_room && pop ? pop_n : 4'd0) < cap;
    end
