 ...
 * drive. Exercised headlessly against hello.rp6502 — no window required.
 */
 ...
 * (hello exits), with the engine no longer reporting stopped. */
UTEST(dbg, continue_runs_to_exit)
{
    ...
    /* hello.rp6502 prints a line and exits within a handful of frames. */
    for (int i = 0; i < 30 && !emu_cpu_halted; i++)
        emu_run_frame();
