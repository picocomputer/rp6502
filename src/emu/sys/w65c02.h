...
/*#
    ...
    This is a 65C02-extended variant of floooh's cycle-stepped NMOS m6502
    core (https://github.com/floooh/chips/). The full W65C02S documented
    instruction set is implemented (BRA, PHX/PLX/PHY/PLY, STZ, INC/DEC A,
    (zp) indirect addressing, BIT #/zp,X/abs,X, TSB/TRB, JMP (abs,X),
    RMB/SMB/BBR/BBS, plus CMOS NOP fills, the CMOS JMP (abs) page-boundary
    fix, and the CMOS BRK decimal-flag clear). WAI and STP are implemented
    as safe stalls (no JAM).

    The PUBLIC API is intentionally kept identical to chips/m6502.h: the
    types m6502_t / m6502_desc_t, the functions m6502_init / m6502_reset
    (n/a) / m6502_tick, and all M6502_* pin macros keep their names so the
    emulator integrates unchanged. This file uses the same include guard /
    CHIPS_IMPL convention and is meant to be included in exactly one
    translation unit; it must NOT be included alongside chips/m6502.h.

    MOS Technology 6502 / 6510 CPU emulator (NMOS base).

    Project repo: https://github.com/floooh/chips/

    NOTE: the NMOS base of this file is code-generated from m6502.template.h
    and m6502_gen.py in the 'codegen' directory. The 65C02 decoder here was
    regenerated for the W65C02S and is hand-maintained in this fork.
...
static inline void _m6502_adc(m6502_t* cpu, uint8_t val) {
    if (cpu->bcd_enabled && (cpu->P & M6502_DF)) {
        ...
        cpu->A = (ah<<4) | (al & 0x0F);
        /* 65C02: N and Z reflect the final BCD-adjusted accumulator (NMOS
         * derives them from the binary intermediate). The accumulator and
         * carry are computed identically to NMOS. */
...
static inline void _m6502_sbc(m6502_t* cpu, uint8_t val) {
    if (cpu->bcd_enabled && (cpu->P & M6502_DF)) {
        ...
        cpu->A = (ah<<4) | (al & 0x0F);
        /* 65C02: N and Z reflect the final BCD-adjusted accumulator (NMOS
         * derives them from the binary difference). The accumulator and
         * carry are computed identically to NMOS. */
        cpu->P = _M6502_NZ(cpu->P, cpu->A);
    }
    else {
...
uint64_t m6502_tick(m6502_t* c, uint64_t pins) {
    ...
    /* BRK */
        ...
        case (0x00<<3)|4: _SA(c->AD++);c->P|=(M6502_IF|M6502_BF);c->P&=~M6502_DF;c->brk_flags=0;break;
    ...
    /* NOP # */
        case (0x02<<3)|0: _SA(c->PC++);_FETCH();break;
    ...
    /* TSB zp */
        ...
        case (0x04<<3)|2: c->AD=_GD();_WR();break;
        case (0x04<<3)|3: if((c->A&c->AD)==0){c->P|=M6502_ZF;}else{c->P&=~M6502_ZF;}c->AD|=c->A;_SD(c->AD);_WR();break;
    ...
    /* RMB0 */
        ...
        case (0x07<<3)|3: c->AD&=~0x1;_SD(c->AD);_WR();break;
    ...
    /* TSB abs */
        ...
        case (0x0C<<3)|3: c->AD=_GD();_WR();break;
        case (0x0C<<3)|4: if((c->A&c->AD)==0){c->P|=M6502_ZF;}else{c->P&=~M6502_ZF;}c->AD|=c->A;_SD(c->AD);_WR();break;
    ...
    /* BBR0 */
        ...
        case (0x0F<<3)|3: if((c->AD&0x1)==0){c->PC=c->PC+(int8_t)_GD();}_FETCH();break;
        case (0x0F<<3)|4: _FETCH();break;
    ...
    /* TRB zp */
        ...
        case (0x14<<3)|2: c->AD=_GD();_WR();break;
        case (0x14<<3)|3: if((c->A&c->AD)==0){c->P|=M6502_ZF;}else{c->P&=~M6502_ZF;}c->AD&=~c->A;_SD(c->AD);_WR();break;
    ...
    /* RMB1 */
        ...
        case (0x17<<3)|2: c->AD=_GD();_WR();break;
        case (0x17<<3)|3: c->AD&=~0x2;_SD(c->AD);_WR();break;
    ...
    /* TRB abs */
        ...
        case (0x1C<<3)|3: c->AD=_GD();_WR();break;
        case (0x1C<<3)|4: if((c->A&c->AD)==0){c->P|=M6502_ZF;}else{c->P&=~M6502_ZF;}c->AD&=~c->A;_SD(c->AD);_WR();break;
    ...
    /* BBR1 */
        ...
        case (0x1F<<3)|3: if((c->AD&0x2)==0){c->PC=c->PC+(int8_t)_GD();}_FETCH();break;
    ...
    /* NOP # */
        case (0x22<<3)|0: _SA(c->PC++);_FETCH();break;
    ...
    /* RMB2 */
        ...
        case (0x27<<3)|3: c->AD&=~0x4;_SD(c->AD);_WR();break;
    ...
    /* BBR2 */
        ...
        case (0x2F<<3)|3: if((c->AD&0x4)==0){c->PC=c->PC+(int8_t)_GD();}_FETCH();break;
        case (0x2F<<3)|4: _FETCH();break;
    ...
    /* BIT zp,X */
        ...
        case (0x34<<3)|3: _m6502_bit(c,_GD());_FETCH();break;
    ...
    /* RMB3 */
        ...
        case (0x37<<3)|2: c->AD=_GD();_WR();break;
        case (0x37<<3)|3: c->AD&=~0x8;_SD(c->AD);_WR();break;
    ...
    /* BIT abs,X */
        ...
        case (0x3C<<3)|4: _m6502_bit(c,_GD());_FETCH();break;
    ...
    /* BBR3 */
        ...
        case (0x3F<<3)|3: if((c->AD&0x8)==0){c->PC=c->PC+(int8_t)_GD();}_FETCH();break;
    ...
    /* NOP # */
        case (0x42<<3)|0: _SA(c->PC++);_FETCH();break;
    ...
    /* BBR4 */
        ...
        case (0x4F<<3)|3: if((c->AD&0x10)==0){c->PC=c->PC+(int8_t)_GD();}_FETCH();break;
        case (0x4F<<3)|4: _FETCH();break;
    ...
    /* NOP zp,X */
        ...
        case (0x54<<3)|2: _SA((c->AD+c->X)&0x00FF);_FETCH();break;
    ...
    /* RMB5 */
        ...
        case (0x57<<3)|2: c->AD=_GD();_WR();break;
    ...
    /* NOP abs,X */
        ...
        case (0x5C<<3)|2: _SA(c->PC);break;
        case (0x5C<<3)|3: _SA(0xFF00|((c->AD)&0xFF));break;
        case (0x5C<<3)|4: _SA(0xFFFF);break;
        case (0x5C<<3)|5: _SA(0xFFFF);break;
        case (0x5C<<3)|6: _SA(0xFFFF);break;
    ...
    /* BBR5 */
        ...
        case (0x5F<<3)|3: if((c->AD&0x20)==0){c->PC=c->PC+(int8_t)_GD();}_FETCH();break;
    ...
    /* NOP # */
        case (0x62<<3)|0: _SA(c->PC++);_FETCH();break;
    ...
    /* JMP (abs) */
        ...
        case (0x6C<<3)|3: _SA((c->AD+1)&0xFFFF);c->AD=_GD();break;
    ...
    /* BBR6 */
        ...
        case (0x6F<<3)|3: if((c->AD&0x40)==0){c->PC=c->PC+(int8_t)_GD();}_FETCH();break;
        case (0x6F<<3)|4: _FETCH();break;
    ...
    /* ADC (zp) */
        ...
        case (0x72<<3)|4: _m6502_adc(c,_GD());_FETCH();break;
    ...
    /* RMB7 */
        ...
        case (0x77<<3)|2: c->AD=_GD();_WR();break;
    ...
    /* JMP (abs,X) */
        ...
        case (0x7C<<3)|2: c->AD|=_GD()<<8;c->AD=(c->AD+c->X)&0xFFFF;_SA(c->PC);break;
        case (0x7C<<3)|3: _SA(c->AD);break;
    ...
    /* BBR7 */
        ...
        case (0x7F<<3)|3: if((c->AD&0x80)==0){c->PC=c->PC+(int8_t)_GD();}_FETCH();break;
    ...
    /* SMB0 */
        ...
        case (0x87<<3)|2: c->AD=_GD();_WR();break;
        case (0x87<<3)|3: c->AD|=0x1;_SD(c->AD);_WR();break;
    ...
    /* BIT # */
        ...
        case (0x89<<3)|1: if((c->A&_GD())==0){c->P|=M6502_ZF;}else{c->P&=~M6502_ZF;}_FETCH();break;
    ...
    /* BBS0 */
        ...
        case (0x8F<<3)|3: if((c->AD&0x1)!=0){c->PC=c->PC+(int8_t)_GD();}_FETCH();break;
        case (0x8F<<3)|4: _FETCH();break;
    ...
    /* SMB1 */
        ...
        case (0x97<<3)|2: c->AD=_GD();_WR();break;
        case (0x97<<3)|3: c->AD|=0x2;_SD(c->AD);_WR();break;
    ...
    /* BBS1 */
        ...
        case (0x9F<<3)|3: if((c->AD&0x2)!=0){c->PC=c->PC+(int8_t)_GD();}_FETCH();break;
    ...
    /* SMB2 */
        ...
        case (0xA7<<3)|2: c->AD=_GD();_WR();break;
        case (0xA7<<3)|3: c->AD|=0x4;_SD(c->AD);_WR();break;
    ...
    /* BBS2 */
        ...
        case (0xAF<<3)|3: if((c->AD&0x4)!=0){c->PC=c->PC+(int8_t)_GD();}_FETCH();break;
        case (0xAF<<3)|4: _FETCH();break;
    ...
    /* SMB3 */
        ...
        case (0xB7<<3)|2: c->AD=_GD();_WR();break;
        case (0xB7<<3)|3: c->AD|=0x8;_SD(c->AD);_WR();break;
    ...
    /* BBS3 */
        ...
        case (0xBF<<3)|3: if((c->AD&0x8)!=0){c->PC=c->PC+(int8_t)_GD();}_FETCH();break;
        case (0xBF<<3)|4: _FETCH();break;
    ...
    /* BBS4 */
        ...
        case (0xCF<<3)|3: if((c->AD&0x10)!=0){c->PC=c->PC+(int8_t)_GD();}_FETCH();break;
        case (0xCF<<3)|4: _FETCH();break;
    ...
    /* CMP (zp) */
        ...
        case (0xD2<<3)|4: _m6502_cmp(c, c->A, _GD());_FETCH();break;
    ...
    /* NOP zp,X */
        ...
        case (0xD4<<3)|2: _SA((c->AD+c->X)&0x00FF);_FETCH();break;
    ...
    /* SMB5 */
        ...
        case (0xD7<<3)|2: c->AD=_GD();_WR();break;
    ...
    /* NOP abs */
        ...
        case (0xDC<<3)|2: _SA((_GD()<<8)|c->AD);_FETCH();break;
    ...
    /* BBS5 */
        ...
        case (0xDF<<3)|3: if((c->AD&0x20)!=0){c->PC=c->PC+(int8_t)_GD();}_FETCH();break;
    ...
    /* BBS6 */
        ...
        case (0xEF<<3)|3: if((c->AD&0x40)!=0){c->PC=c->PC+(int8_t)_GD();}_FETCH();break;
        case (0xEF<<3)|4: _FETCH();break;
    ...
    /* SBC (zp) */
        ...
        case (0xF2<<3)|4: _m6502_sbc(c,_GD());_FETCH();break;
    ...
    /* NOP zp,X */
        ...
        case (0xF4<<3)|2: _SA((c->AD+c->X)&0x00FF);_FETCH();break;
    ...
    /* SMB7 */
        ...
        case (0xF7<<3)|2: c->AD=_GD();_WR();break;
    ...
    /* NOP abs */
        ...
        case (0xFC<<3)|2: _SA((_GD()<<8)|c->AD);_FETCH();break;
        case (0xFC<<3)|3: assert(false);break;
    ...
    /* BBS7 */
        ...
        case (0xFF<<3)|3: if((c->AD&0x80)!=0){c->PC=c->PC+(int8_t)_GD();}_FETCH();break;
