...
static bool hid_receive_report(uint8_t dev_addr, uint8_t instance)
{
    if (tuh_hid_receive_report(dev_addr, instance))
        return true;
    dev_printf(dev_addr, "?HID unable to receive report on devce %d instance %d", dev_addr, instance);
    return false;
}
...
void tuh_hid_mount_cb(uint8_t dev_addr, uint8_t instance, uint8_t const *desc_report, uint16_t desc_len)
{
    ...
    if (has_keyboard && has_mouse && other_reports)
        dev_printf(dev_addr, "HID keyboard, mouse, and %d other reports", other_reports);
    else if (has_keyboard && other_reports)
        dev_printf(dev_addr, "HID keyboard and %d other reports", other_reports);
    else if (has_mouse && other_reports)
        dev_printf(dev_addr, "HID mouse and %d other reports", other_reports);
    else if (has_keyboard && has_mouse)
        dev_printf(dev_addr, "HID keyboard and mouse");
    else if (has_keyboard)
        dev_printf(dev_addr, "HID keyboard");
    else if (has_mouse)
